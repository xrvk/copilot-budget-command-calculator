...
|---------|-----------------|--------------|
| **Enterprise budget** | After pool depletion | Hard ceiling on metered charges once the pool runs dry. Zero effect while pool capacity remains. |
| **Cost center budget** | After pool depletion | Per-team cap on metered charges. Useful for chargeback, but cannot protect a team's share of the pre-paid pool. |
| **Universal user-level budget (ULB)** | Always | Caps each person's total monthly consumption (pool + metered). Primary fairness control. |
| **Individual user-level budget** | Always | A higher personal cap for specific named users who demonstrably need more. An override on the universal ULB. |

### 5. Cost center exclusion

One toggle fundamentally changes how the enterprise budget and cost center budgets interact:

- **Exclusion OFF (default):** The enterprise budget is the single umbrella covering all metered charges. Cost center budgets act as sub-limits within it. Best for organizations where a small number of enterprise admins manage all budgets centrally.
...
### 1. Always set a universal user-level budget

Without one, a single user or agent can consume the entire enterprise pool overnight. The universal ULB is your primary fairness control — it caps each user's total monthly AI credit consumption (pool + metered combined).
...
### 3. Always enable "Stop usage when budget limit is reached"

The "Stop usage when budget limit is reached" toggle (`prevent_further_usage` API flag) turns a budget into a hard stop. Without it, every budget is advisory — usage and charges continue past the limit uncapped.

### 4. Size the enterprise budget from your seat mix

The enterprise budget is a metered-charges safety net, not a total budget. It's derived from your ULB settings and pool size: total max consumption minus pool value equals your potential metered spend. Add a buffer, and that's your enterprise budget. It does nothing while the pool has capacity.
...
### 1. Raise individual ULBs before upgrading license tiers

An individual ULB on a Business license lets a user borrow from the pool at no extra cost. During standard pricing, upgrading adds $20/seat for a net-zero AIC gain. Only consider an upgrade if the user consistently maxes out their individual ULB month after month.

### 2. Gate individual ULB increases on prior-month usage data

Individual ULBs don't expand the pool — they raise the per-user ceiling, accelerating depletion for everyone. Require usage data before granting increases. Power user status should be demonstrated, not self-reported.
...
**New to Copilot budgeting?** Start by importing your enterprise data in the **Budget Planner** tab. Once connected, the **Tier Planner** will calculate optimal user-level budgets, enterprise budget, and cost center budgets for your configuration.
