...
import type { CsvUserUsage } from '../lib/chargeback'
import {
...
function makeUser(login: string, totalAICs: number, quota = 1000): CsvUserUsage {
  return { login, totalAICs, grossAmount: totalAICs * 0.01, netAmount: 0, costCenter: null, organization: null, totalMonthlyQuota: quota }
}
...
describe('calcConsumptionStats', () => {
  ...
  it('counts CB and CE seats by quota', () => {
    const users = [
      makeUser('a', 100, 300),
      makeUser('b', 200, 300),
      makeUser('c', 300, 1000),
...
describe('applyThreshold', () => {
  ...
  it('handles threshold that includes all users', () => {
    const result = applyThreshold(users, 10) // everyone is above 10
    expect(result.powerUserCount).toBe(5)
    expect(result.regularUserCount).toBe(0)
    expect(result.suggestedULB).toBe(0) // no regular users
  })
...
describe('calcThreshold', () => {
  const users = Array.from({ length: 10 }, (_, i) =>
    makeUser(`user-${i}`, (i + 1) * 100),
  )
  // users: 100, 200, ..., 1000

  it('top-10 gets roughly 1 power user from 10', () => {
    const result = calcThreshold(users, 'top-10')
    expect(result.powerUserCount).toBe(1)
  })

  it('top-20 gets roughly 2 power users from 10', () => {
    const result = calcThreshold(users, 'top-20')
    expect(result.powerUserCount).toBe(2)
  })

  it('top-30 gets roughly 3 power users from 10', () => {
    const result = calcThreshold(users, 'top-30')
    expect(result.powerUserCount).toBe(3)
  })

  it('custom mode uses provided threshold', () => {
    const result = calcThreshold(users, 'custom', 500)
    expect(result.powerUserCount).toBe(6) // 500-1000
...
describe('detectLicenseMix', () => {
  it('counts business and enterprise seats', () => {
    const users = [
      makeUser('a', 100, 300),
      makeUser('b', 200, 300),
      makeUser('c', 300, 1000),
    ...
    expect(mix.ceSeats).toBe(3)
  })

  it('returns 0 for unknown quotas', () => {
    const users = [makeUser('a', 100, 500)] // unknown quota
    const mix = detectLicenseMix(users)
