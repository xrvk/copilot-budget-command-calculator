            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="universal-ulb" className="flex items-center gap-2">
                  ...
                  Universal User Budget (All Users)
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className={`p-6 rounded-lg bg-card border-2 ${tierBorder} text-center`}>
                <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground mb-2">
                  {tier === 'hard' ? 'Max AIC Spend' : tier === 'soft' ? 'AIC Alert Threshold' : tier === 'blind' ? 'AIC Spend (uncapped)' : 'Suggested AIC Cap'}
                  <FormulaTooltip
                    title="Max AIC Spend (post-pool)"
                    side="bottom"
                    steps={[
                      {
                        label: tier === 'hard'
                          ? 'Max post-pool AI Credit charges (capped by budgets)'
                          : tier === 'soft'
                            ? 'Post-pool AI Credit charges (alert threshold only, not enforced)'
                            : tier === 'blind'
                              ? 'Post-pool AI Credit charges (no budget controls active)'
                              : 'Post-pool AI Credit charges (no enterprise budget set yet)',
                        formula: effectiveExcludeCostCenterUsage && hasCostCenters
                          ...
                          : `Enterprise limit $${displayEntBudget.toLocaleString()} (covers all usage)`,
            ...
            <div className="space-y-3 pt-2">
                  ...
                  <div className="flex justify-between items-center p-3 rounded bg-muted">
                    <div className="flex items-center gap-2 text-base font-semibold">
                      Max AIC Charges (post-pool)
                      <FormulaTooltip
                        title="Max AIC Charges (post-pool)"
                        steps={[
                          {
                          ...
                          {
                            label: tier !== null ? 'Enterprise limit (actual on GitHub)' : `Enterprise limit (+ ${enterpriseBufferPercent}% buffer, rounded up)`,
                            formula: tier !== null
                  ...
                  <div className="text-sm text-muted-foreground px-3 space-y-1">
                    ...
                    {maxSpendBeyondReservoir === 0 && (
                      <p className="text-success">Pool covers all usage. No post-pool charges expected</p>
                    )}
                  </div>
                </div>
