      ...
      <div className="flex-1 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          {tier === 'hard'
            ...
            : tier === 'soft'
              ? `Set to alert when metered charges reach this amount. Enable Stop usage to enforce a hard cap`
              : tier === 'blind'
        ...
        {tier === 'blind' && !isReservoirSufficient && (
          <Alert className="border-destructive/50 bg-destructive/10 py-2">
            ...
            <AlertDescription className="text-xs">
              <strong>Not enforced.</strong> Neither alerts nor stop usage are enabled, so metered charges are uncapped. Enable <em>Stop usage when budget limit is reached</em> in Budget Planner.
            </AlertDescription>
          </Alert>
        )}
        {tier === 'blind' && isReservoirSufficient && (
                      ...
                      <AlertDescription className="text-xs space-y-1">
                        ...
                        <p className="text-muted-foreground">
                          {effectiveExcludeCostCenterUsage
                            ? `With cost center exclusion on, this budget only covers ${regularUsers} non-cost-center users. Their metered charges are capped at $${liveEntBudget.toLocaleString()}, which may block them before they reach their $${universalULB} ULB`
                            : `This budget caps all ${totalUsers} users' metered charges. Users may be blocked before reaching their ULB ($${universalULB}) or individual ULBs ($${powerUserBudget})`
                          }
                        </p>
                      </AlertDescription>
                    </Alert>
