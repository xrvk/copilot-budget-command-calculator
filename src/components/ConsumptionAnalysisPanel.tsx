              ...
              {budgetAdjusted && (
                <Alert className="border-warning/40 bg-warning/5">
                    ...
                    <AlertDescription className="text-xs space-y-2">
                      <p>
                        <span className="font-semibold text-foreground">Suggested ULBs scaled down to fit your enterprise budget.</span>
                        {' '}If every user consumed up to their ULB, last month's data would suggest <span className="font-semibold">${bufferedULB.toLocaleString()}</span> for base users and <span className="font-semibold">${bufferedPowerBudget.toLocaleString()}</span> for power users. Pool ${poolValueUsd.toLocaleString()} + enterprise budget ${enterpriseBudget?.toLocaleString()} caps total spend at <span className="font-semibold">${(poolValueUsd + (enterpriseBudget ?? 0)).toLocaleString()}</span>. The shown ULBs are {Math.round((1 - budgetScalingFactor) * 100)}% lower to fit.
                      </p>
                      <p className="text-muted-foreground">
                        <span className="font-semibold text-foreground">Realistic spend is typically much lower.</span>
                        {' '}The Tier Planner shows a forecast based on last month's actual pattern. The pool is derived from CSV-active users only · seat holders with zero usage are absent, so if your real seat count is higher the ceiling shown is conservative.
                      </p>
                    </AlertDescription>
                  </Alert>
              )}
