...
import { getHashParams, setHash } from '@/lib/hash-routing'
import {
  ArrowLeft,
  ShieldCheck,
  Key,
...
function scrollToSection(id: string, updateUrl = true) {
  ...
  if (updateUrl) {
    // Preserve existing hash params (e.g. popup=0) when updating section
    const existing = getHashParams()
    existing.set('section', id)
    const merged: Record<string, string> = {}
    existing.forEach((value, key) => { merged[key] = value })
    setHash('docs', merged)
  }
}
  ...
  return (
    <div className="flex gap-8 max-w-6xl mx-auto px-4 sm:px-6 py-8">
      ...
      <section id="tier-planner" className="mb-12 scroll-mt-20 pt-12 border-t border-border/50">
        ...
        <p className="text-muted-foreground mb-4">
          Configure every layer of Copilot's budget hierarchy, from the enterprise budget down to individual user-level budgets. The planner calculates optimal values from your seat counts and consumption targets, then executes each change via the API.
        </p>
        ...
        <h3 className="text-lg font-semibold mb-3">The guided workflow</h3>
        <p className="text-muted-foreground mb-4">
        ...
        <div className="space-y-4 mb-6">
          <StepCard
            ...
            description="Set or verify the enterprise budget. This caps metered charges (charges beyond the included AI Credit pool). The app calculates the minimum budget needed to support all users at their configured consumption levels, plus your buffer percentage."
          />
          <StepCard
            number={2}
          ...
          <StepCard
            ...
            description="Bulk-create individual user-level budgets for power users. These override the universal ULB for specific users, giving them higher consumption limits. Select an enterprise team, review the member list, and apply budgets in batch."
          />
          <StepCard
            number={5}
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent className="text-sm text-muted-foreground space-y-3">
              ...
              <p>
                <strong className="text-foreground">Step 1: Determine each user's included credits.</strong>{' '}
              ...
              <p>
                ...
                For each user: if they used more than their included credits, the difference is their additional usage. Users who stayed within their included credits owe nothing: they only consumed from the pool already paid for via their seat fee. In formula form: additional_usage = max(0, actual_usage − included_credits).
              </p>
              <p>
              ...
              <p>
                ...
                If a user consumed at or below their included credits, they did not contribute to metered charges. Their usage was fully covered by the pre-paid pool.
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="text-sm text-muted-foreground space-y-2">
              <ul className="list-disc list-inside space-y-1.5 ml-1">
                ...
                <li><strong className="text-foreground">Underutilizers subsidize over-consumers.</strong> When a user consumes below their included credits, their unused AICs flow into the shared pool. The subsidizing department receives no credit for this.</li>
      ...
      <section id="limitations" className="scroll-mt-20 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="text-sm text-muted-foreground space-y-3">
              <p>
                This project was created and maintained by a sole GitHub Solutions Engineer to help customers navigate usage-based billing. It is not an officially supported GitHub product and is provided as-is with no guarantees of support, accuracy, or financial guidance.
              </p>
              <p>
                This tool makes real API calls that modify your enterprise billing configuration. You are responsible for reviewing all changes before applying them. GitHub may change its billing API, pricing, or credit allocations at any time, and this tool may not reflect those changes immediately.
              </p>
              <p className="font-medium text-foreground">
                ⚠ Still stuck?{' '}
                <a
                  href="https://github.com/xrvk/copilot-budget-command-calculator/issues"
                  target="_blank"
                ...
                . Never include personal or company data in the issue.
              </p>
            </CardContent>
          </Card>
