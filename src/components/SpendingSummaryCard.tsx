    ...
    <Card className={`border-2 shadow-md ${cardClass}`}>
      <CardHeader>
        ...
        <CardDescription>
          ...
          {tier === 'blind' && 'No enterprise cap or notifications. Universal ULB (if set) is the only backstop, capping each user\'s total spend (pool + metered)'}
        </CardDescription>
      </CardHeader>
        ...
        {tier === 'soft' && (
          <Alert className="border-warning/50 bg-warning/10">
            ...
            <AlertDescription className="text-xs">
              {hasUncappedGap ? (
                <>
                  <strong>Partial cap.</strong> Stop usage is on at the enterprise budget, but {uncappedCcCount} excluded cost center{uncappedCcCount !== 1 ? 's have' : ' has'} no budget. They have no per-team cap on metered charges. Universal ULB (if set) is the only backstop, capping each user's total spend (pool + metered). Add a CC budget for a true per-team limit.
                </>
              ) : (
                <>
